-- Constants:
-- user_presence is realtime data — anything older than 15 minutes is stale.
-- Other "session" tables still use is_active = false.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'counts', jsonb_build_object(
      'logs', (SELECT COUNT(*)::bigint FROM public.audit_log WHERE created_at < v_cutoff)
      ...
      'sessions', (SELECT COUNT(*)::bigint FROM public.active_sessions WHERE is_active = false)
                ...
                + (SELECT COUNT(*)::bigint FROM public.user_presence WHERE last_seen_at < v_presence_cutoff),
      'ratelimits', (SELECT COUNT(*)::bigint FROM public.rate_limit_attempts WHERE first_attempt_at < v_cutoff),
      'notifications', (SELECT COUNT(*)::bigint FROM public.admin_messages WHERE expires_at < v_now)
    ...
    'table_counts', jsonb_build_object(
      ...
      'user_presence', (SELECT COUNT(*)::bigint FROM public.user_presence WHERE last_seen_at < v_presence_cutoff),
...
BEGIN
  ...
  IF v_section IN ('logs', 'all') THEN
    LOOP
      WITH batch AS (SELECT ctid FROM public.audit_log WHERE created_at < v_cutoff LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'audit_log');

    LOOP
      WITH batch AS (SELECT ctid FROM public.admin_access_logs WHERE created_at < v_cutoff LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'admin_access_logs');

    LOOP
      WITH batch AS (SELECT ctid FROM public.admin_audit_logs WHERE created_at < v_cutoff LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'admin_audit_logs');

    LOOP
      WITH batch AS (SELECT ctid FROM public.employee_action_logs WHERE created_at < v_cutoff LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'employee_action_logs');
  END IF;

  IF v_section IN ('sessions', 'all') THEN
    LOOP
      WITH batch AS (SELECT ctid FROM public.active_sessions WHERE is_active = false LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'active_sessions');

    LOOP
      WITH batch AS (SELECT ctid FROM public.pdv_sessions WHERE is_active = false LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'pdv_sessions');

    LOOP
      WITH batch AS (SELECT ctid FROM public.unidade_sessions WHERE is_active = false LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'unidade_sessions');
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'user_presence');
  END IF;
  ...
  IF v_section IN ('ratelimits', 'all') THEN
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'rate_limit_attempts');
  END IF;
  ...
  IF v_section IN ('notifications', 'all') THEN
    LOOP
      WITH expired_messages AS (
        ...
        DELETE FROM public.admin_message_recipients r USING expired_messages em
        WHERE r.message_id = em.id RETURNING 1
      )
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'admin_message_recipients');

    LOOP
      WITH batch AS (SELECT ctid FROM public.admin_messages WHERE expires_at < v_now LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'admin_messages');

    LOOP
      WITH expired_notifications AS (
        SELECT id FROM public.global_notifications WHERE expires_at < v_now LIMIT v_batch_size
      ), deleted_recipients AS (
        DELETE FROM public.global_notification_recipients r USING expired_notifications en
        WHERE r.notification_id = en.id RETURNING 1
      )
      SELECT COUNT(*) INTO v_deleted FROM deleted_recipients;
      EXIT WHEN v_deleted = 0;
      v_total_deleted := v_total_deleted + v_deleted;
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'global_notification_recipients');

    LOOP
      WITH batch AS (SELECT ctid FROM public.global_notifications WHERE expires_at < v_now LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'global_notifications');

    LOOP
      WITH batch AS (SELECT ctid FROM public.admin_realtime_messages WHERE expires_at < v_now LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'admin_realtime_messages');

    LOOP
      WITH batch AS (SELECT ctid FROM public.user_direct_messages WHERE expires_at < v_now LIMIT v_batch_size)
    ...
    END LOOP;
    v_touched_tables := array_append(v_touched_tables, 'user_direct_messages');
  END IF;

  -- Reclaim storage: REINDEX touched tables (concurrently to avoid locking writes).
  -- This shrinks the bloated indexes (and therefore total_relation_size) so the UI shows the new size.
  FOREACH v_tbl IN ARRAY v_touched_tables LOOP
    BEGIN
      EXECUTE format('REINDEX TABLE public.%I', v_tbl);
    EXCEPTION WHEN OTHERS THEN
      -- Don't fail the whole cleanup if reindex hits a transient lock; the rows are already gone.
      NULL;
    END;
  END LOOP;
...
CREATE OR REPLACE FUNCTION public.cleanup_system_table(p_table text, p_retention_days integer DEFAULT 30, p_batch_size integer DEFAULT 5000)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Reclaim storage by rebuilding the indexes for the touched table.
  BEGIN
    EXECUTE format('REINDEX TABLE public.%I', v_table);
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
